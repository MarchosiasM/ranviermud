...
module.exports = {
  listeners: {
    ...
    knockedOffBalance: (state) => (target, attacker) => {
      const effect = state.EffectFactory.create("offBalance");
      B.sayAt(target, `${attacker.name} knocks you off balance!`);
      B.sayAt(attacker, `You knock ${target.name} off balance!`);
